...
test('submits the kansspelautomaat aanwezigheidsvergunning eHerkenning happy flow', { tag: '@kvk-69599084' }, async ({ page }, testInfo) => {
  ...
  await captureLoadedFormState('kansspelautomaat-aanwezig-vergunning-eherkenning-01-startpagina');
  ...
  await page.getByLabel('Inschrijfnummer Kamer van Koophandel (KvK)', { exact: true }).fill(testCompany.kvkNumber);
  ...
  await captureLoadedFormState('kansspelautomaat-aanwezig-vergunning-eherkenning-02-aanvraag');
  ...
  await captureLoadedFormState('kansspelautomaat-aanwezig-vergunning-eherkenning-03-bijlagen');
  await page.getByRole('button', { name: 'Volgende', exact: true }).click();
  await expectFormStep(page, 'Contactgegevens');

  await page.getByRole('radio', { name: 'ja', exact: true }).check();
  await page.getByLabel('Naam contactpersoon', { exact: true }).fill('Test contactpersoon Playwright');
  ...
  await captureLoadedFormState('kansspelautomaat-aanwezig-vergunning-eherkenning-04-contactgegevens');
  await page.getByRole('button', { name: 'Volgende', exact: true }).click();
  await expectFormStep(page, 'Controleer en bevestig');

  await captureLoadedFormState('kansspelautomaat-aanwezig-vergunning-eherkenning-05-controleer-en-bevestig');
  ...
  console.log(`Form artifacts: ${await captureFormState(page, testInfo, 'kansspelautomaat-aanwezig-vergunning-eherkenning-99-bevestiging')}`);
});
